...
class FractalthornsAPI(API):
    ...
    def __init__(self):
        ...
        __single_image = Request(
            self.ValidRequests.SINGLE_IMAGE.value, [RequestArgument("name", True)]
        )

        __image_description = Request(
            self.ValidRequests.IMAGE_DESCRIPTION.value, [RequestArgument("name", False)]
        )
        ...
        __single_record = Request(
            self.ValidRequests.SINGLE_RECORD.value, [RequestArgument("name", False)]
        )

        __record_text = Request(
            self.ValidRequests.RECORD_TEXT.value, [RequestArgument("name", False)]
        )
        ...
        self.__cached_news_items: Optional[Tuple[List[Dict[str, str]], dt.datetime]] = (
            None
        )
        self.__cached_images: Dict[
            str, Tuple[Dict[str, Union[str, int, bool, List[str], None]], dt.datetime]
        ] = {}
        ...
        self.__cached_image_descriptions: Dict[
            str, Tuple[Optional[str], dt.datetime]
        ] = {}
        self.__cached_chapters: Optional[
            Tuple[Dict[str, List[Dict[str, Union[str, bool, None]]]], dt.datetime]
        ] = None
        self.__cached_records: Dict[
            str, Tuple[Dict[str, Union[str, bool, None]], dt.datetime]
        ] = {}
        ...
        self.__metadata_cache_purge_allowed: Tuple[dt.datetime, str] = (
            dt.datetime.now(dt.UTC),
        )
        self.__contents_cache_purge_allowed: Tuple[dt.datetime, str] = (
            dt.datetime.now(dt.UTC),
        )
    ...
    def _make_request(
        self,
        endpoint: str,
        request_payload: Optional[Dict[str, str]],
        *,
    ...
    def purge_metadata_cache(self, *, force_purge: bool = False):
        ...
        if (
            force_purge
            or dt.datetime.now(dt.UTC) > self.__metadata_cache_purge_allowed[0]
        ):
            ...
            if self.__metadata_cache_purge_allowed[0] < new_time:
                self.__metadata_cache_purge_allowed = (
                    new_time,
    ...
    def purge_contents_cache(self, *, force_purge: bool = False):
        ...
        if (
            force_purge
            or dt.datetime.now(dt.UTC) > self.__contents_cache_purge_allowed[0]
        ):
            ...
            if self.__contents_cache_purge_allowed[0] < new_time:
                self.__contents_cache_purge_allowed = (
                    new_time,
                    self.InvalidPurgeReasons.CACHE_PURGE.value,
                )
        else:
    ...
    def get_all_news(
        self, *, formatting: Dict[str, bool] = None, start_index=0, amount=3
    ) -> str:
    ...
    def get_single_image(
        self, name: Optional[str], *, formatting: Dict[str, bool] = None
    ...
    def get_single_record(
        self, name: str, *, formatting: Dict[str, bool] = None
    ) -> str:
    ...
    def get_domain_search(
        self, term: str, type_: str, *, start_index: int = 0, amount: int = 10
    ):
            ...
            match item["type"]:
                ...
                case "episodic-item":
                    results_join_list.append(
                        self.__format_single_record_inline(item["record"])
                    )
                ...
                case "episodic-line":
                    ...
                    if item["record"]["solved"]:
                        matching_line = (
                            self.__get_record_text(item["record"]["name"])
                        )["lines"][item["record_line_index"]]
                        matching_text = matching_line["text"]
                        matching_text = matching_text.replace(
                            item["record_matched_text"],
                            f"**{item['record_matched_text']}**",
                        )
                        matching_line["text"] = matching_text
                        record_text = self.__format_record_line(
                            matching_line, None, None
                        )[0]
    ...
    def __get_images_list(self) -> List[str]:
        if (
            self.__valid_image_urls is None
    ...
    def __get_records_list(self) -> List[str]:
        if (
            self.__valid_record_urls is None
            or dt.datetime.now(dt.UTC) > self.__valid_record_urls[1]
        ):
    ...
    def __get_news_items(self) -> List[Dict[str, str]]:
        if (
            ...
            or dt.datetime.now(dt.UTC) > self.__cached_news_items[1]
        ):
    ...
    def __get_image_contents(self, image: str) -> Tuple[Image.Image, Image.Image]:
        ...
        if (
            ...
            image_req = requests.get(
                "".join((self._base_url, image_metadata["image_url"])),
            ...
            thumb_req = requests.get(
                "".join((self._base_url, image_metadata["thumb_url"])),
                timeout=self.__REQUEST_TIMEOUT,
                headers=self.__DEFAULT_HEADERS,
            )
    ...
    def __get_image_description(self, image: str) -> Optional[str]:
        ...
        if (
            ...
            self.__cached_image_descriptions.update(
                {
                    image: (
                        image_description.get("description"),
                        dt.datetime.now(dt.UTC) + self.__CONTENTS_CACHE_DURATION,
                    )
                }
            )
    ...
    def __get_full_episodic(self) -> Dict[str, List[Dict[str, Union[str, bool, None]]]]:
        if (
            self.__cached_chapters is None
            or dt.datetime.now(dt.UTC) > self.__cached_chapters[1]
            or self.__valid_record_urls is None
            or dt.datetime.now(dt.UTC) > self.__valid_record_urls[1]
        ):
    ...
    def __get_single_record(self, name: str) -> Dict[str, Union[str, bool, None]]:
        ...
        if (
            ...
            or dt.datetime.now(dt.UTC) > self.__cached_records[name][1]
        ):
    ...
    def __get_domain_search(self, term, type_) -> List[
        Dict[
            str,
            Union[
                str,
    ...
    def __format_news_item(
        self, item: Dict[str, str], formatting: Dict[str, bool]
    ) -> str:
        news_join_list = []
    ...
    def __format_single_image(
        ...
        for format_ in formatting.keys():
            ...
            if format_ == "ordinal" and formatting[format_]:
                image_join_list.append(
                    "".join(("> _(image #", str(item["ordinal"]), ")_"))
                )
            ...
            if format_ == "has_description" and formatting[format_]:
                has_description = " ".join(
                    ("> has description:", "yes" if item["has_description"] else "no")
                )
            ...
            if format_ == "speedpaint_video_url" and formatting[format_]:
                if item.get("speedpaint_video_url") is not None:
                    speedpaint = "".join(
                        ("> [speedpaint video](<", item["speedpaint_video_url"], ">)")
                    )
                else:
    ...
    def __format_full_episodic(
        self, item: Dict[str, List[Dict[str, Union[str, bool, None]]]]
    ) -> str:
        episodic_join_list = []
        ...
        for chapter in item.keys():
            ...
            for record in item[chapter]:
                episodic_join_list.append(
                    self.__format_single_record_inline(record, show_chapter=False)
                )

        return ("\n".join(episodic_join_list)).lstrip()

    def __format_single_record_inline(
        self,
        record: Dict[str, Union[str, bool, None]],
        *,
    ...
    def __format_single_record(
        self, item: Dict[str, Union[str, bool, None]], formatting: Dict[str, bool]
    ) -> str:
        record_join_list = []
        name_done = False
        ...
        for format_ in formatting.keys():
            ...
            if format_ == "solved" and formatting[format_]:
                solved = "".join(
                    ("> _solved: ", "yes" if item["solved"] else "no", "_")
                )
    ...
    def __format_record_text(
        self,
        item: Tuple[
            str, Dict[str, Union[bool, str, List[str], List[Optional[str]], None]]
        ],
    ) -> str:
        record_title = item[0]
        ...
        pre_header = "".join(
            ("> (_iteration: ", record_contents["iteration"], "; language(s): ")
        )
        ...
        for line in record_contents["lines"]:
            line_string, last_character, last_language = self.__format_record_line(
                line, last_character, last_language
            )
